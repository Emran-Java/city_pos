 ...
 */
public class BankMenuItemsFragment extends BaseFragment {
    /**
     * menu page row
     */
    private static int PAGE_ROW = 7;
    ...
    private AppFragmentBankMenuItemsBinding binding;

    @NonNull
    public static BankMenuItemsFragment newInstance() {
        return new BankMenuItemsFragment();
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = AppFragmentBankMenuItemsBinding.inflate(inflater, container, false);
        ...
        MenuChangeListen.getInstance().getMemo().observe(getViewLifecycleOwner(), memo -> {
            if(memo){
                MainMenu.getInstance().changeBracMenu(memo);
                initView();
            }
        });
...
*/
    ...
    private void initView() {
        ...
        //menu page
        List<InnerBankIMenuItemsFragment> childFragments = createMenuFragments(items);
        binding.viewPagerMenuItems.setAdapter(new FragmentStateAdapter(this) {
            @NonNull
            @Override
        ...
        if (childFragments.size() < 2) {
            ...
            binding.viewPagerMenuItems.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
                @Override
    ...
    @NonNull
    private List<InnerBankIMenuItemsFragment> createMenuFragments(@NonNull List<MenuItem> items) {
        List<InnerBankIMenuItemsFragment> fragments = new ArrayList<>();
        ...
        for (int i = 0; i < items.size(); i += pageSize) {
            ...
            if(BDevice.supportPhysicalKeyboard() && heightDps<=ScreenHeightDps.HEIGHT_285_DPS){
                fragments.add(InnerBankIMenuItemsFragment.newInstance(fragItems, this::resetSaver));
            }else {
                fragments.add(InnerBankIMenuItemsFragment.newInstance(fragItems));
            }
...
//                .setConfirmButton(dialog -> {})
    ...
    public void onResume() {
        ...
        if(!listFragment.isEmpty()){
            ...
            if(topFragment instanceof BankMenuItemsFragment){
                registerN560Key();
            }
        }
    ...
    private class DotAdapter extends BaseBindingRecyclerAdapter<AppPageDotBinding> {
        ...
        protected void bindItemData(AppPageDotBinding itemBinding, int position) {
            ...
            itemBinding.getRoot().setOnClickListener(v -> binding.viewPagerMenuItems.setCurrentItem(position, true));
        }
                        ...
                        if (handleKey == keyCode) {
                            switch (keyCode) {
                                case KeyEvent.KEYCODE_STAR:
                                    int index = binding.viewPagerMenuItems.getCurrentItem();
                                    if(index!=0){
                                        binding.viewPagerMenuItems.setCurrentItem(index-1);
                                        return true;
                                    }
                                    break;
                                case KeyEvent.KEYCODE_POUND:
                                    int currentIndex = binding.viewPagerMenuItems.getCurrentItem();
                                    if(currentIndex!=(Objects.requireNonNull(binding.viewPagerMenuItems.getAdapter()).getItemCount()-1)){
                                        binding.viewPagerMenuItems.setCurrentItem(currentIndex+1);
                                        return true;
                                    }
                                    return true;
                                default:
                                    FragmentStateAdapter adapter = (FragmentStateAdapter) binding.viewPagerMenuItems.getAdapter();
...
//                                        MenuFragment currentFragment = (MenuFragment) adapter.createFragment(binding.viewPager.getCurrentItem());
                                        assert adapter != null;
                                        InnerBankIMenuItemsFragment currentFragment = (InnerBankIMenuItemsFragment) adapter.createFragment(binding.viewPagerMenuItems.getCurrentItem());
