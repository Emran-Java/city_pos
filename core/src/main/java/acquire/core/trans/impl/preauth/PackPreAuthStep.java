...
/**
 * The {@link BaseStep} that packs {@link PreAuthMenu} 8583 and sends them to the server.
 *
 * @author Janson
...
class PackPreAuthStep extends BaseStep {
    ...
    public void intercept(Callback callback)  {
        ...
        pubBean.setMessageId("0100");       // Field 0 MTI
        pubBean.setProcessCode("300000");   // Field 3
        ...
        try {
           ...
           /* if (!TextUtils.isEmpty(pubBean.getCardSn())) {
            ...
            if (!TextUtils.isEmpty(pubBean.getNii())) {
                iso8583.setField(24, pubBean.getNii()); // Network International ID
            }
           /* else{
